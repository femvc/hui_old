 ...
 * @date 2013/08/08
 */
define('./hui.Validator', ['./hui'], function(){
hui.dom = hui.dom || document;
...
hui.Validator = {
     ...
     */
    noticeInTail: function(control, noticeText) {
        var me = hui.Validator,
            input = control && control.getMain ? control.getMain() : (control && control.main ? hui.dom.getElementById(control.main) : control);
        me.showNoticeDom(control);
        var title = input.getAttribute('title') || '';
        me.setInnerHTML(title + noticeText, me.getTextEl(control));
    },
    noticeInTailNoTitle: function(control, noticeText) {
        var me = hui.Validator,
            input = control && control.getMain ? control.getMain() : (control && control.main ? hui.dom.getElementById(control.main) : control);
        me.showNoticeDom(control);
    ...
    showError: function (control, noticeText) {
        this.noticeInTailNoTitle(control, noticeText);
    },
    /**
     * @name 显示notice的dom元素
     * @private
    ...
    showNoticeDom: function(control) {
        ...
        hui.Validator.addClass(father, me.config['errorClass']);
    },
    ...
    cancelNoticeInTile: function(control) {
        ...
        hui.Validator.removeClass(father, me.config['errorClass']);
    ...
    hideError: function (control) {
        this.cancelNoticeInTile(control);
    },
    /**
     * @name 获取info区域的元素
     * @private
    ...
    getTextEl: function(input) {
        var me = this,
            ...
            control = input,
            input = control && control.getMain ? control.getMain() : (control && control.main ? hui.dom.getElementById(control.main) : control);
        elem = hui.dom.getElementById(input.id +'-'+ me.config['textSuffix']);
        return elem;
    },
    ...
    getEl: function(input) {
        var me = this,
            ...
            control = input,
            input = control && control.getMain ? control.getMain() : (control && control.main ? hui.dom.getElementById(control.main) : control);
        elem = hui.dom.getElementById(input.id +'-'+ me.config['suffix']);
        return elem;
    },
    /**
     * @name 获取提示元素
    ...
    getSegElement: function(control, segId) {
        var me = this,
            elem = hui.dom.getElementById(segId);
        return elem;
    },
    /**
     * @name 验证规则
    ...
    applyRule: function(control, ruleName) {
        ...
        // [nodejs&browser]
        hui.dom = control.getMain().setInnerHTML ? hui.bocument : document;
        ...
        if (parseInt('0'+String(error).replace(/\s/g,''),10) !== 0) { //TODO:这种形式是要被历史遗弃的
            ...
            errorText = hui.Validator.format(error[0], error.splice(1,error.length));
        }

        if (errorText) {
            rule.notice = rule.notice || me.noticeInTail;
            rule.notice(control, errorText);
        } 
        else {
        ...
        for (i = 1; i < segLen; i++) {
            ...
            // preserveArgument 保护参数
                ...
                if (hui 
                    && hui.Control 
                    && hui.Control.getById 
                    && (ctrl = hui.Control.getById(ruleSegment[i], control.getAction ? control.getAction() : null))
                    && !!ctrl) {
 ...
 */
hui.Validator.format = function (source, opts) {
 ...
 */
hui.Validator.addClass = function (element, className) {
    hui.Validator.removeClass(element, className);
    element.className = (element.className +' '+ className).replace(/(\s)+/ig," ");
    return element;
};
hui.Validator.removeClass = function(element, className) {
...
*/   
hui.Validator.parseDate = function(str){   
...
hui.Validator.ruleMap = {
    ...
    'username': {
        ...
        'notice': hui.Validator.noticeInTail,
        'cancelNotice': hui.Validator.cancelNoticeInTile,
        'noticeText': {
            1: 'Can not be empty',
    ...
    'emailVerify': {
        ...
        'notice': hui.Validator.noticeInTailNoTitle,
        'cancelNotice': hui.Validator.cancelNoticeInTile,
        'noticeText': {
 ...
 * @param {Map} rule 规则内容
 */
hui.Validator.addRule = function(ruleName, ruleContent, force){
    if (!force && hui.Validator.ruleMap[ruleName]) {
        throw { title: 'hui.Validator.addRule() Error: ', name: 'rule "'+ruleName+'" exist.'};
    }
    else {
