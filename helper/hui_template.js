 ...
 * @date 2013/08/08
 */
define('./hui.Template', ['./hui'], function(){
...
hui.Template = {
    ...
    merge: function(targetContent, model) {
        ...
        targetContent = String(targetContent).replace(
            ...
            function($0, $1) {
                ...
                if (variable === undefined && hui && hui.context && hui.context.get) { 
                    variable = hui.context.get(varName);
                }
    ...
    parseVariableByType: function(varName, type) {
        ...
        if (len == 1 && hui && hui[type]) {
            variable = hui[type][packs[0]];
        } 
        else {
            variable = hui.window;
            while (len--) {
