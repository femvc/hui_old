 ...
 */
define('./hui.ButtonGroup', ['./hui', './hui.Control'], function(){

hui.ButtonGroup = function (options, pending) {
    hui.ButtonGroup.superClass.call(this, options, 'pending');
...
hui.ButtonGroup.prototype = {
    ...
    getTpl: function () {
        var tpl = '<button type="button" id="#{0}" value="#{1}" class="btn btn-info #{2}" ' + 
            ' onclick="hui.Control.getById(\'#{3}\').setValue(this.getAttribute(\'value\'))" >#{1}</button>';
    ...
    initContent: function() {
        ...
        for (var i=0,len=options.length; i<len; i++) {
            item = options[i];
            html.push(hui.Control.format(me.getTpl(),
                me.getId(item),
                item,
                item == value ? ' active' : '',
                me.getId()
    ...
    initBehavior: function () {
        var me = this,
            main = me.getMain();
        (window.console&&window.console.log('initBehavior - main:' + main.id));
    },
    /**
     * @name 渲染控件
     * @protected
     */
    render: function() {
        hui.ButtonGroup.superClass.prototype.render.call(this);
        var me = this;
    ...
    setValue: function (value) {
        ...
        for (var i=0,len=options.length; i<len; i++) {
            ...
            if (value != undefined && item.getAttribute('value') === String(value)) {
                hui.Control.addClass(item, 'active');
            }
            else {
                hui.Control.removeClass(item, 'active');
            }
        };
...
//hui.Control.derive(hui.ButtonGroup);
hui.inherits(hui.ButtonGroup, hui.Control);
